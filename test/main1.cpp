...
const string WINDOW_TITLE = "GuessIt";
...
int getPlayerGuess()
{
    ...
    while (number<=0 || number>MAX_NUM) {
        ...
        if (e.type == SDL_MOUSEBUTTONDOWN) {
            number = computerNumber(e.button.x, e.button.y);
        }
    }
    return number;
}
...
void load_SDL_and_Images()
{
    ...
    bigMsg = loadTexture("images/big_old.bmp", renderer);
    smallMsg = loadTexture("images/small_old.bmp", renderer);
